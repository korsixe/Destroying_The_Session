...
\begin{proof}
    Пусть $h_1, \dots, h_S$ --- собственные векторы, отвечающие различным собственным 
    ...
    (\lambda_1 - \lambda_s) \cdot h_1 = 0 + ... + 0\]
...
\begin{proof}
    ...
    корнями которого они являются. Рассмотрим преобразование \[B = A^2 + p A + q E\]
    ...
    \[h \in \Ker B \Longrightarrow B(h) = \lambda_1^2 h + p \lambda_1 h + q h = (\lambda_1^2 + p \lambda_1 + q) \cdot h = 0 
    \Longrightarrow \lambda_1^2 + p \lambda_1 + q = 0 \ (\text{так как } h \ne 0)\]  
    Но уравнение $\lambda_1^2 + p \lambda_1 + q = 0$ имеет лишь комплексные корни, а $\lambda_1 \in \mathbb{R}$.
...
\begin{theorem}[Достаточное условие диагонализируемости]
    ...
    все корни характеристического многочлена вещественны и различны.
\end{theorem}
